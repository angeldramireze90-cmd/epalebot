...
def detectar_categoria(texto):
    ...
    for cat, palabras in CATEGORIAS.items():
        p = sum(1 for w in palabras if w in t)
        if p > 0:
            puntajes[cat] = p
...
def get_ws(nombre, headers):
    ...
    try:
        ...
        try:
            ...
            if existing != headers:
                logging.warning(f"Headers incorrectos en [{nombre}], corrigiendo...")
                ws.clear()
...
HDRS_DON = ["ID","FECHA","FECHA_MATCH","USER_ID","USERNAME","NOMBRE",
            "ESTADO_VE","ZONA","DESCRIPCION","CATEGORIA","ESTADO","MATCH_ID","REPUTACION"]
...
(TIPO, ESTADO_VE, ZONA, DESCRIPCION, CONFIRMAR, CALIFICAR_STEP) = range(6)
...
async def recibir_zona(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Si tenés varias cosas, sepараlas con coma.\n\n"
...
async def recibir_descripcion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("✏️ Cambiar descripción",  callback_data="cambiar_desc")],
        [InlineKeyboardButton("📍 Cambiar ubicación",    callback_data="cambiar_ubic")],
...
async def confirmar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "cambiar_desc":
        await query.edit_message_text("Escribí la nueva descripción:")
        return DESCRIPCION
    ...
    ids_guardados = []
    logging.info(f"CONFIRMAR: usuario={uid} tipo={tipo} items={len(items)}")
    for item_desc, item_cat in items:
        item_id = f"{'DON' if tipo=='donar' else 'NEC'}-{datetime.now().strftime('%Y%m%d%H%M%S')}-{next(_id_counter)}"
        logging.info(f"Guardando item: {item_id} - {item_desc} - {item_cat}")
        resultado = await guardar(hoja, hdrs, [item_id, fecha, "", uid, uname, nombre,
                             estado_ve, zona, item_desc, item_cat, "Disponible", "", 0])
        logging.info(f"Resultado guardar: {resultado}")
...
class HealthHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"EpaleBot activo")
